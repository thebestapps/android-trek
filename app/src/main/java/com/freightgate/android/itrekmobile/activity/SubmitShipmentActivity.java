...
//		super.onBackPressed();
	...
	public void setFormData(ArrayList<Shipment> shipments){
		...
		if(shipments.size() > 0){
			...
			shipper += "\n";
			shipper += "SHP REF#: "+(shipments.get(0).getShp_ref().trim().length()>0?shipments.get(0).getShp_ref().substring(0, shipments.get(0).getShp_ref().length()):"");
			...
			if(shipments.get(0).getAgent_num().isEmpty()){
				...
				consignee += "\n";
				consignee += "PO#: "+(shipments.get(0).getCsg_ref().trim().length()>0?shipments.get(0).getCsg_ref().substring(0, shipments.get(0).getCsg_ref().length()):"");
